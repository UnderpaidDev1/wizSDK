...
class Card:
    ...
    async def enchant(self, spell):
        ...
        # click self
        await self.client.mouse.click((self.spell_x, self.spell_y), duration=.3, delay=.3)
        # click spell
        await self.client.mouse.click((spell.spell_x, spell.spell_y), duration=.3, delay=.3)
    ...
    async def cast(self, target=None):
        ...
        if target != None:
            if target < 4:
                ...
                print(f'Invalid value for target, expect int between 0 - 7, got {target}')
                return False
            
            await self.client.mouse.click((x, y), duration=.3, delay=.3)
        
"""
Calculate spell_x of enchanted card
"""
# Start with position of spell
# add 1/2 card width
# if position of enchant is less than position of spell
#   subtract 1 card width